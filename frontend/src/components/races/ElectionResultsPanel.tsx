...
function ElectionResultsPanel({ raceId, certificationStatus, civicRaceDetail }: ElectionResultsPanelProps) {
  const [electionResults, setElectionResults] = useState<OfficialResultsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const electionResultsRequest = civicElectionsApi
    ...
    void Promise.allSettled([electionResultsRequest, mockTallyRequest])
      .then(([electionResultsResponse, mockTallyResponse]) => {
        if (!isActive) {
          return;
        }

        if (electionResultsResponse.status === 'fulfilled') {
          setElectionResults(electionResultsResponse.value);
        } else if (certificationStatus !== 'results_pending') {
          setError(getApiErrorMessage(electionResultsResponse.reason, 'We could not load election results right now.'));
        }
  ...
  const effectiveCertificationStatus = electionResults?.certification_status ?? certificationStatus;
  const rows = electionResults?.results ?? [];
  ...
  const sourceUrl = electionResults?.source_url || rows[0]?.source_url || '';
